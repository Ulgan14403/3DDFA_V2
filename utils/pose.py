...
def P2sRt(P):
    ...
    Args:
        P: (3, 4). Affine Camera Matrix. <- composants prédits
    Returns:
        s: scale factor.
        R: (3, 3). rotation matrix. <- matrice d'euler, obtenue à partir des composants prédits
...
def matrix2angle(R):
    ...
    return x, y, z #ORDRE : yxz, [x =beta (pitch), y=alpha(yaw),z=gamma(roll)]
...
def eular2matrix(phi, gamma, theta):
    rx = [[1, 0, 0],
          [0, cos(phi), sin(phi)],
          [0, -sin(phi), cos(phi)]]
    ...
    ry = np.array(ry, dtype=np.float32)
    rz = np.array(rz, dtype=np.float32)
